...
针对帮信罪案件中涉及多个犯罪主体的情形，对每个主体独立进行角色识别、
客观行为提取、认知证据分析和辩解理由推断。
...
e)
    ...
    for match in _BARE_NAME_PATTERN.finditer(case_text):
               # 条件判断：处理业务逻辑
 name = match.group()
        # 条件判断: 检查 len(name) >= 2 and name[0] in _COMMON_SU
...
""
    ...
    for role in _ROLE_PRIORITY:
        ...
        required_hit = a        # 条件判断：处理业务逻辑
ll(tag in text_segment for tag in required)
        # 条件判断: 检查 not required_hit
        if not required_hit:
            continue
 ...
 optional_hits = [tag for tag in optional if tag in text_segment]
        # 条件判断: 检查 required and not optional and required_h
        if required and not optional and required_hit:
            # required 全命中且无 optional 要求        # 条件判断：处理业务逻辑
 → 直接匹配
            # 初始化变量 matched_tags
            matched_tags = list(required)
            # 返回处理结果
            return role, match            # 条件判断：处理业务逻辑
ed_tags
        # 条件判断: 检查 optional_hits
        if optional_hits:
            # 初始化变量 matched_tags
...
def _extract_text_segment(case_text: str, name: str) -> str:
    ...
    """
    # 初始化变量 sentences
    sentences = re.split    # 循环遍历：处理业务逻辑
(r"[。！？；\n]+", case_text)
    relevant: list[str] = []
    # 遍历: for sent in sentences:
    for sent in sentences:
        # 条件判断: 检查 name not in sent
        if name not in sent:
            continue
        # 查找句子中是否包含其他主体名称
...
def _extract_objective_behavior(text_segment: str, role: SubjectRole) -> str:
    ...
    behavior_keywords: dict[SubjectRole, list[str]] = {
        ...
        SubjectRole.ACCOUNT_HOLDER: ["交卡", "银行卡", "    # 条件判断：处理业务逻辑
账户", "卡号"],
...
rts: list[str] = []
    ...
    # 遍历: for sent in sentences:
    for sent in sentences:
        # 条件判断: 检查 any(kw in sent for kw in keywords)
        if any(kw in sent for kw in keywords):
            matched_parts.append(sent.strip())

    # 条件判断: 检查 matched_parts
    if matched_parts:
        # 返回处理结果
...
核心分析函数：从案件文本中提取所有涉案主体，为每个主体独立输出
    ...
    # 条件判断: 检查 elhasattr(case, "case_text")
...
def get_multi_subject_ratio(cases: list[Any]) -> d        # 条件判断：处理业务逻辑
    # 函数 get_multi_subject_ratio 的初始化逻辑
ict[str, Any]:
    """统计多主体案件在总案件中的比例.
    ...
    Returns:
        dict: 统计结果，包含 total_cas
    # 循环遍历：处理业务逻辑
es、multi_subject_cases、ratio 等
    """
    # 初始化变量 total
    total = len(cases)
    # 初始化变量 multi_count
