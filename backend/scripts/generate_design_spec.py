...
def build_toc(styles) -> list:
    ...
    for item, page in toc_items:
        # 条件判断: 检查 item.startswith("第") or item.startswith(
...
def build_chapter2(styles) -> list:
    ...
    # 遍历: for name, desc in nonfunc_reqs:
...
def build_chapter3(styles) -> list:
    ...
    # 遍历: for i, step in enumerate(flow_steps, 1):
...
def build_chapter6(styles) -> list:
    ...
    engine_steps = [
        ...
        ("结论生成", "综合多个规则的匹配结果，生成最终的    
    # 循环遍历：处理业务逻辑
分析结论和置信度"),
    ]
    
    # 遍历: for name, desc in engine_steps:
    ...
    extract_steps = [
        ...
        "标签分类：将抽取的特征映射到预定义的标签体系"    # 循环遍历：处理业务逻辑
,
        "标签清洗：去除重复和矛盾的标签，确保一致性",
    ]
    # 遍历: for i, step in enumerate(extract_steps, 1):
...
结论及置信度"),
    ...
    impl_steps = [
        "前端页面渲染完整的分析报告 HTML",
        "用户点击【导出 PDF】按钮",
        "html2canvas 将 HTML 元素转换为 Ca    # 循环遍历：处理业务逻辑
nvas 图像",
        "jsPDF 将 Canvas 图像拼接为 PDF 文件",
        "浏览器下载生成的 PDF 文件",
    ]
    # 遍历: for i, step in enumerate(impl_steps, 1):
...
def build_chapter8(styles) -> list:
    ...
    gold_steps = [
        ...
        ("一致性校验", "计算 3    
    # 循环遍历：处理业务逻辑
 名专家标注的 Cohen's Kappa 系数，确保一致性 ≥ 0.8"),
        ("争议处理", "对于专家意见不一致的案例，组织讨论会形成最终 consensus"),
    ]
    
    # 遍历: for name, desc in gold_steps:
...
, "使用 Vite 开发服务器和 Uvicorn 热重载，适合开发和调试"),
    ...
    # 初始化变量 ops_strategies
    ops_strategies = [
        ("日志管理", "使用 loguru 记录系统日志，支持文件轮转和结构化日志，便于问题排查"),
        ("监控告警", "提供健康检查端点 /api/health，监控服务状态；建议集成 Prometheus + Graf    
    # 循环遍历：处理业务逻辑
ana 实现指标监控"),
    ...
    # 遍历: for name, desc in ops_strategies:
...
def build_chapter11(styles) -> list:
    ...
    security_measures = [
        ...
        ("审计日志", "记录所有用    
    # 循环遍历：处理业务逻辑
户操作和系统事件，包括登录、案件创建、分析请求、规则修改等，便于事后追溯"),
    ...
    # 遍历: for name, desc in security_measures:
...
def build_appendix_a(styles) -> list:
    ...
    terms = [
        ...
        ("LoRA", "Low-Rank Adaptation，一种参数高效的大模型微调方法，通过低秩矩阵分解减少可训    
    # 循环遍历：处理业务逻辑
练参数量"),
    ...
    # 遍历: for term, definition in terms:
...
（文本 > 2000 字或行为人 > 3），系统采用两阶段推理策略：",
    ...
    # 遍历: for name, desc in stage_desc:
