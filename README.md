...
| 用户认证与权限 | 已完成 | 安全团队 | - |
| 数据加密存储 | 已完成 | 安全团队 | - |
| API 限流保护 | Beta | 运维团队 | 2026-Q3 |
| 模型微调支持 | Beta | AI团队 | 2026-Q3 |
| 批量案件处理 | 计划中 | 后端团队 | 2026-Q4 |
| 多模型对比分析 | 计划中 | AI团队 | 2026-Q4 |

---

## 已知限制

1. **暂不支持批量案件处理**，原因是当前架构设计为单案件串行分析，批量处理需要引入任务队列和并发控制机制，预计在 2026-Q4 版本实现。

2. **暂不支持多模型并行对比**，原因是 Ollama 本地部署资源有限，并行推理会导致显存不足，需要升级硬件或引入模型服务集群。

3. **暂不支持实时协作标注**，原因是当前标注系统为单人模式，多人协作需要引入 WebSocket 实时通信和冲突解决机制。

4. **暂不支持自定义分析维度**，原因是三维度分析模型经过司法专家验证，自定义维度可能影响分析结果的法律效力。

5. **暂不支持离线部署**，原因是系统依赖 Ollama 服务进行模型推理，完全离线需要本地部署完整模型服务栈。
